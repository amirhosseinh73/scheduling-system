	...
	var MDTimePicker = /*#__PURE__*/function () {
		...
		function MDTimePicker(el, config) {
			...
			this.timepicker = {
				...
				clockHolder: {
					...
					buttonsHolder: {
						...
						}, (!config.btnOkContent) ? DEFAULTS.btnOkContent : config.btnOkContent),
						btnCancel: hf.createElem('span', {
							class: 'mdtp__button cancel'
						}, (!config.btnCancelContent) ? DEFAULTS.btnCancelContent : config.btnCancelContent),
					}
				}
			};
			...
			if (_.input.value !== '') {
				...
				var _time = (!config.defaultTime) ? _.getSystemTime() : config.defaultTime;

				_.time = new Time(_time.hour, _time.minute);
				//edited by amirhosein
				//_.setValue(_time);
			}
